...
use nautilus_model::{data::prices::MarkPriceUpdate, identifiers::InstrumentId};
...
mod tests {
    ...
    fn test_decode_batch() {
        ...
        let record_batch = crate::arrow::record_batch_with_timestamps(
            crate::arrow::schema_without_identifier_column(&MarkPriceUpdate::get_schema(Some(
                metadata.clone(),
            )))
            .into(),
    ...
    fn test_decode_batch_rejects_null_timestamp_with_field_and_row() {
        ...
        let encoded = MarkPriceUpdate::encode_batch(&metadata, &[update]).unwrap();
        ...
        let error = MarkPriceUpdate::decode_batch(&metadata, batch).unwrap_err();
