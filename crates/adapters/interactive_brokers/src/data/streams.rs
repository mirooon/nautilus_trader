...
async fn run_stream<T, Subscribe, Process>(
...
where
    T: Send + 'static,
    Subscribe:
        for<'a> FnMut(&'a StreamContext) -> StreamFuture<'a, anyhow::Result<Subscription<T>>>,
    Process: for<'a> FnMut(
        &'a mut Subscription<T>,
        &'a StreamContext,
        u64,
...
async fn process_historical_bar_stream(
    subscription: &mut Subscription<HistoricalBarUpdate>,
    context: &StreamContext,
    farm_generation: u64,
    bar_type: BarType,
    handle_revised_bars: bool,
...
async fn process_market_data_stream<Process>(
...
async fn process_tick_by_tick_quote_stream(
    subscription: &mut Subscription<ibapi::market_data::realtime::BidAsk>,
    context: &StreamContext,
    farm_generation: u64,
    price_magnifier: f64,
    mut monitor: SubscriptionMonitor,
            ...
            tick_result = subscription.next() => {
                match tick_result {
                    Some(Ok(SubscriptionItem::Data(bid_ask))) => {
                        ...
                        if is_sentinel_price(bid_ask.bid_price)
                            || is_sentinel_price(bid_ask.ask_price)
            ...
            tick_result = subscription.next() => {
                match tick_result {
                    Some(Ok(SubscriptionItem::Data(tick))) => {
                        ...
                        if is_sentinel_price(tick.price)
                            || !tick.size.is_finite()
                            || tick.size <= 0.0
                        {
                            tracing::debug!(
    ...
    E: Debug,
{
    match tick_result.map(IntoSubscriptionTick::into_subscription_item) {
        ...
        {
            if is_sentinel_price(price.price) {
                tracing::debug!(
        ...
        {
            if is_sentinel_price(price_size.price) {
                tracing::debug!(
                    "Ignoring IB index price sentinel for {}: {}",
                    instrument_id,
                    price_size.price
...
mod tests {
    ...
    use super::{
        AtomicTime, ClientId, DataFarmConnectionState, DataFarmIdentity, DataFarmKind,
        DataFarmRecoveryScope, InteractiveBrokersSubscriptionIdle, OptionGreeksCache, QuoteCache,
        StreamAction, StreamConfig, SubscriptionMonitor, process_index_price_tick_result,
    ...
    async fn test_process_trade_stream_emits_negative_price_and_skips_sentinel() {
        ...
        match receiver.recv().await.unwrap() {
            DataEvent::Data(Data::Trade(trade)) => {
                assert_eq!(trade.price.as_f64(), -1.25);
                assert_eq!(trade.size.as_f64(), 3.0);
            }
            other => panic!("unexpected event: {other:?}"),
        }
