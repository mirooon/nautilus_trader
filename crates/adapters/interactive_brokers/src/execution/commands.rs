...
impl InteractiveBrokersExecutionClient {
    ...
    pub(super) async fn handle_submit_order_list_async(
        ...
        for order in orders {
            ...
            {
                let state = order_tracker.lock()?;
                anyhow::ensure!(
                    state.order_id_map.contains_key(&parent_order_id),
        ...
        for (index, order) in orders.iter().enumerate() {
                ...
                let parent_ib_order_id =
                    ib_order_ids.get(&parent_order_id).copied().or_else(|| {
                        order_tracker
                            .lock()
                            .ok()
                            .and_then(|state| state.order_id_map.get(&parent_order_id).copied())
                    });
            ...
            if let Err(e) = client
                ...
                .await
            {
                if let Err(denial_error) = Self::deny_unsubmitted_order_list_tail(
                    &orders[index + 1..],
                    strategy_id,
                    exec_sender,
                    clock,
    ...
    pub(super) fn deny_unsubmitted_order_list_tail(
        orders: &[OrderAny],
        strategy_id: StrategyId,
        exec_sender: &EventSender<ExecutionEvent>,
        clock: &'static AtomicTime,
