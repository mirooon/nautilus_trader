...
impl InteractiveBrokersExecutionClient {
    ...
    fn handle_accepted_order_notice(
        ...
        if let Some(venue_ctx) = venue_ctx {
            Self::spawn_accepted_order_presence_check(
                venue_ctx,
                order_id,
                client_order_id,
                orders.clone(),
    ...
    fn spawn_accepted_order_presence_check(
        ...
        let future = async move {
            tokio::select! {
                () = Self::check_accepted_order_presence(
                    &client,
                    ib_order_id,
                    client_order_id,
                    timeout_dur,
        ...
        if let Err(e) = venue_ctx.tasks.spawn(future) {
            tracing::warn!("Skipping IB accepted-order presence check after shutdown began: {e}");
        }
    }

    async fn check_accepted_order_presence(
        client: &Arc<Client>,
        ib_order_id: i32,
        client_order_id: ClientOrderId,
        ...
        let subscription = match tokio::time::timeout(timeout_dur, client.all_open_orders()).await {
            ...
            Ok(Err(e)) => {
                tracing::error!(
                    "Presence check for {client_order_id} failed to request open orders: {e}"
                );
                return;
            }
            Err(_) => {
                tracing::error!(
                    "Presence check for {client_order_id} timed out requesting open orders"
                );
                return;
            }
        };
        ...
        Self::resolve_absent_accepted_order(
            client,
            ib_order_id,
            client_order_id,
            timeout_dur,
    ...
    // Queries completed orders for the terminal status of an accepted order that
    // vanished from open orders after a venue notice, and emits the inferred
    // terminal event so the engine does not hold a dead order as working.
    #[allow(clippy::too_many_arguments)] // Venue resolution shares the notice context.
    async fn resolve_absent_accepted_order(
        client: &Arc<Client>,
        ib_order_id: i32,
        client_order_id: ClientOrderId,
        timeout_dur: Duration,
        ...
        let subscription = match tokio::time::timeout(timeout_dur, client.completed_orders(false))
            .await
        {
            Ok(Ok(subscription)) => subscription,
            Ok(Err(e)) => {
                tracing::error!(
                    "Order {client_order_id} is absent from open orders and the completed-orders query failed: {e}; venue state needs reconciliation"
                );
                return;
            }
            Err(_) => {
                tracing::error!(
                    "Order {client_order_id} is absent from open orders and the completed-orders query timed out; venue state needs reconciliation"
                );
                return;
            }
        };
        ...
        let mut completed: Option<(OrderStatusKind, i64)> = None;

        while let Some(order_result) = subscription.next().await {
            if let Ok(Orders::OrderData(data)) = order_result
                && (data.order_id == ib_order_id
                    || (data.order.perm_id != 0 && Some(data.order.perm_id) == tracked_perm_id))
            {
                completed = Some((data.order_state.status, data.order.perm_id));
                break;
            }
        }
        ...
        let event = {
            ...
            let Some(tracked) = state.order(ib_order_id) else {
                tracing::debug!(
                    "Order {client_order_id} is no longer tracked; absent-order resolution done"
                );
                return;
            };
    ...
    // Maps a completed-orders status for an absent accepted order to the inferred
    // terminal event, or None when no event can be honestly inferred.
    pub(super) fn absent_order_terminal_event(
        completed: Option<(OrderStatusKind, i64)>,
        tracked: &TrackedOrder,
        ib_order_id: i32,
        account_id: AccountId,
        ts_init: UnixNanos,
    ) -> Option<OrderEventAny> {
        let (status, perm_id) = completed?;
